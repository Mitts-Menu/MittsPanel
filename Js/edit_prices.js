...
function loadMenuData() {
    ...
    Promise.all([
        ...
        if (trMenuData.length === 0) {
            container.innerHTML = "<p>Menü verisi bulunamadı.</p>";
            return;
        }
...
function updateItemPrice(categoryId, itemId, newPrice, trMenuData, enMenuData) {
    ...
    const trCategory = trMenuData.find(cat => cat.category_id === categoryId);
    if (trCategory) {
        const trItemIndex = trCategory.items.findIndex(item => item.id === itemId);
        if (trItemIndex !== -1) {
            trCategory.items[trItemIndex].price = newPrice;
            updatePromises.push(db.ref(`menu/tr/${trCategory._key}/items/${trItemIndex}/price`).set(newPrice));
        }
    }
    const enCategory = enMenuData.find(cat => cat.category_id === categoryId);
    if (enCategory) {
        const enItemIndex = enCategory.items.findIndex(item => item.id === itemId);
        if (enItemIndex !== -1) {
            enCategory.items[enItemIndex].price = newPrice;
            updatePromises.push(db.ref(`menu/en/${enCategory._key}/items/${enItemIndex}/price`).set(newPrice));
        }
    }
